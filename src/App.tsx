...
function App() {
	...
	//sections heights
	const homeActive: number = homeRef.current
		? homeRef.current.offsetTop + homeRef.current.clientHeight - (window.innerHeight * 15) / 100
		: 0;
	const aboutActive: number = aboutRef.current
		? aboutRef.current.offsetTop + aboutRef.current.clientHeight - (window.innerHeight * 15) / 100
		: 0;
	const practiceActive: number = practiceRef.current
		? practiceRef.current.offsetTop +
		  practiceRef.current.clientHeight -
		  (window.innerHeight * 15) / 100
		: 0;
	const contactActive: number = contactRef.current
		? contactRef.current.offsetTop +
		  contactRef.current.clientHeight -
		  (window.innerHeight * 15) / 100
		: 0;
	//sections top
	const homeTop: number = homeRef.current
		? homeRef.current.offsetTop - (window.innerHeight * 15) / 100
		: 0;
	const aboutTop: number = aboutRef.current
		? aboutRef.current.offsetTop - (window.innerHeight * 14) / 100
		: 0;
	const practiceTop: number = practiceRef.current
		? practiceRef.current.offsetTop - (window.innerHeight * 14) / 100
		: 0;
	const contactTop: number = contactRef.current
		? contactRef.current.offsetTop - (window.innerHeight * 14) / 100
		: 0;
	...
	React.useEffect(() => {
		if (sectionActive === 0) {
			window.scrollTo({
				top: homeRef.current ? homeRef.current.offsetTop - (window.innerHeight * 15) / 100 : 0,
				behavior: 'smooth',
			});
		} else if (sectionActive === 2) {
			window.scrollTo({
				top: aboutRef.current ? aboutRef.current.offsetTop - (window.innerHeight * 14) / 100 : 0,
				behavior: 'smooth',
			});
		} else if (sectionActive === 3) {
			window.scrollTo({
				top: practiceRef.current
					? practiceRef.current.offsetTop - (window.innerHeight * 14) / 100
					: 0,
				behavior: 'smooth',
			});
	...
	const [indexOpenedBlog, setIndexOpenedBlog] = useState<{
		title: string;
		author: string;
		time: string;
		date: string;
	...
	return (
		<Router>
					...
					<Route
						...
						element={
							<>
								...
								<Box ref={contactRef}>
									<Contact
										contactActive={contactRef.current ? contactRef.current.clientHeight : 0}
									/>
								</Box>
								<Footer />
							</>
          ...
          <Button variant='contained'sx={{background:'#48504B',color:'#FFFFFF',fontFamily: "'Raleway', sans-serif",borderRadius:'0',height:'8vh',width:'13vw',fontSize:'1.1rem',display:'flex',alignItems:'center'}}>CHAT WITH US!</Button>
				...
				<Box
					position='fixed'
					className={scrollTopVisible ? 'app__res__scrollToTop active' : 'app__res__scrollToTop'}
					display={{ xs: 'block', md: 'none' }}
